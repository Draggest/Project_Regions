# Фукнция создания интерактивной карты для альтернативного решения
def create_map_2(llist,llist2):
    import os
    import json
    import random
    import requests
    ...
    new_data2 = {"type": "FeatureCollection",
    "features": []}
    ...
    for el in nlist:
        for feature in data['features']:
            if el == feature['properties']['name']:
                new_data2['features'].append(feature)
    ...
    geo_json3 = GeoJSON(
        ...
        style_callback = color2
    )

    m.add_layer(geo_json)
    m.add_layer(geo_json2)
    m.add_layer(geo_json3)
    return m